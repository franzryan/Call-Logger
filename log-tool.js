...
function copyLogs() {
  ...
  let placeHolder = document.getElementById('place-holder').innerHTML
  ...
  function copyBtns(clickedBtn) {
    ...
    for(identity in btnObject) { //for in loop of arra
      if (btnId == btnObject[identity].id) //condition if clicked buttons's matches the content of array property's id */
  ...
  let chatOneDiv = document.getElementById("chat-one-div");
  let chatTwoDiv = document.getElementById("chat-two-div");
  let chatSelector = document.getElementsByClassName("chat-selector");